...
window.addEventListener('load', () => {
  ...
  */
  ...
  class CoCreateYjsCodemirror {
    ...
    _initYSocket(element, editor, initData) {
      ...
      if (collection && name && document_id) {
        ...
        this.adapterDB(id, binding.awareness.doc, element);
        new UserCursor(provider);
        if (initData) {
          // g_cocreateY.insertData(id, 0, initData)
        }
      }
    }
    ...
    initEvent(element, editor) {
      ...
      element.addEventListener('set-document_id', function(event){
        ...
        const old_data = editor.getValue();
        _this._initYSocket(element, editor, old_data);
